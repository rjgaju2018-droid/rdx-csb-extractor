...
#  STANDARD IMPORTS
# ══════════════════════════════════════════════════════════════════════════════
import os, re, sys, glob, json, math, hashlib, threading, time, shutil, subprocess, tempfile, webbrowser, csv
...
# ── Multi-Courier Tracker engine (carriers/courier_tracker) ───────────────────
_COURIER_TRACKER_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "carriers", "courier_tracker")
if _COURIER_TRACKER_DIR not in sys.path:
    sys.path.insert(0, _COURIER_TRACKER_DIR)
try:
    from tracker_engine import TrackingEngine, detect_courier
    COURIER_TRACKER_AVAILABLE = True
except ImportError:
    COURIER_TRACKER_AVAILABLE = False
...
class Dashboard(ctk.CTkFrame):
    def __init__(self, master, username, user_id=None, is_admin=False, on_open_processing=None, on_open_courier_tracker=None):
        super().__init__(master, fg_color=DARK_BG)
        self.username = username
        self.user_id = user_id
        self.is_admin = is_admin
        self.on_open_processing = on_open_processing
        self.on_open_courier_tracker = on_open_courier_tracker
        self._build()
    ...
    def _build(self):
        ...
        actions_row.grid(row=6, column=0, sticky="ew", padx=30, pady=(0, 30))
        actions_row.grid_columnconfigure(0, weight=3)
        actions_row.grid_columnconfigure(1, weight=1)
        ctk.CTkButton(actions_row, text="▶  Open Processing Window", height=50, corner_radius=8, fg_color=GOLD_MID, hover_color=GOLD_DARK, text_color=DARK_BG, font=("Segoe UI", 14, "bold"), command=self._open).grid(row=0, column=0, sticky="ew", padx=(0, 8))
        ctk.CTkButton(actions_row, text="🚚  Multi-Courier Tracker", height=50, corner_radius=8, fg_color=CARD_BG, hover_color=GOLD_DARK, text_color=GOLD_LIGHT, border_color=GOLD_DARK, border_width=1, font=("Segoe UI", 13, "bold"), command=self._open_courier_tracker).grid(row=0, column=1, sticky="ew")
    ...
    def _open_courier_tracker(self):
        """Open the Multi-Courier Tracker as a page inside this same window."""
        self._monitoring = False
        if self.on_open_courier_tracker:
            self.on_open_courier_tracker(self.username, self.user_id)
...
# ══════════════════════════════════════════════════════════════════════════════
#  MULTI-COURIER TRACKER PAGE  (embedded, RDX-themed — same window as the rest)
# ══════════════════════════════════════════════════════════════════════════════
COURIER_LIST = ["Auto", "FedEx", "DHL", "UPS", "Aramex", "Shiprocket", "OnPoint", "ShipGlobal", "Xindus"]

class CourierTrackerPage(ctk.CTkFrame):
    def __init__(self, master, username, user_id=None, on_back=None):
        super().__init__(master, fg_color=DARK_BG)
        self.username = username
        self.user_id = user_id
    ...
    def _build(self):
        ...
        header.grid(row=0, column=0, sticky="ew")
        header.grid_columnconfigure(0, weight=1)
        ctk.CTkLabel(header, text="🚚 Multi-Courier Tracker", font=("Segoe UI", 14, "bold"), text_color=GOLD_LIGHT).grid(row=0, column=0, padx=20, sticky="w")
        ctk.CTkButton(header, text="⚙ API Settings", width=120, height=30, fg_color="#3A2E20", text_color=WHITE, hover_color=GOLD_DARK, command=self._open_settings).grid(row=0, column=1, padx=(0, 10))
        ctk.CTkButton(header, text="⬅ Back to Dashboard", width=150, height=30, fg_color=GOLD_DARK, hover_color=GOLD_MID, command=self._back).grid(row=0, column=2, padx=20)

        if not COURIER_TRACKER_AVAILABLE:
            ctk.CTkLabel(self, text="⚠ Multi-Courier Tracker engine not found (carriers/courier_tracker/tracker_engine.py missing\nor the 'requests' package isn't installed — run: pip install requests).",
                         font=("Segoe UI", 12), text_color=RED_ERR, justify="left").grid(row=1, column=0, padx=30, pady=30, sticky="w")
            return

        # Input row
        ops = ctk.CTkFrame(self, fg_color="transparent")
        ops.grid(row=1, column=0, sticky="ew", padx=20, pady=(15, 5))

        ctk.CTkLabel(ops, text="Courier:", font=("Segoe UI", 11), text_color=GREY_TEXT).pack(side="left", padx=(5, 5))
        self.courier_var = tk.StringVar(value="Auto")
        courier_menu = ctk.CTkOptionMenu(ops, values=COURIER_LIST, variable=self.courier_var, width=130,
                                          fg_color=CARD_BG, button_color=GOLD_DARK, button_hover_color=GOLD_MID, text_color=CREAM)
        courier_menu.pack(side="left", padx=(0, 15))

        self.import_btn = ctk.CTkButton(ops, text="📂 Import CSV", width=120, fg_color="#3A2E20", text_color=WHITE, hover_color=GOLD_DARK, command=self._import_csv)
        self.import_btn.pack(side="left", padx=5)

        self.start_btn = ctk.CTkButton(ops, text="🚀 Start Tracking", font=("Segoe UI", 12, "bold"), fg_color=GREEN_OK, text_color=WHITE, hover_color="#66C266", command=self._start_tracking)
        self.start_btn.pack(side="left", padx=5)

        self.stop_btn = ctk.CTkButton(ops, text="⏹ Stop", width=90, fg_color=RED_ERR, text_color=WHITE, hover_color="#C24040", command=self._stop_tracking, state="disabled")
        self.stop_btn.pack(side="left", padx=5)

        self.export_btn = ctk.CTkButton(ops, text="📊 Export Excel", width=130, fg_color=GOLD_MID, text_color=DARK_BG, hover_color=GOLD_LIGHT, command=self._export_excel)
        self.export_btn.pack(side="right", padx=5)

        self.clear_btn = ctk.CTkButton(ops, text="🗑 Clear", width=90, fg_color="#3A2E20", text_color=WHITE, hover_color=RED_ERR, command=self._clear_results)
        ...
        input_frame = ctk.CTkFrame(self, fg_color=CARD_BG, border_color=GOLD_DARK, border_width=1)
        input_frame.grid(row=2, column=0, sticky="ew", padx=20, pady=5)
        ctk.CTkLabel(input_frame, text="Paste tracking numbers (one per line — optionally 'Courier:Number'):",
                     font=("Segoe UI", 10), text_color=GREY_TEXT).pack(anchor="w", padx=10, pady=(8, 2))
        self.input_box = tk.Text(input_frame, height=4, bg=CARD_BG, fg=CREAM, insertbackground=GOLD_LIGHT, font=("Consolas", 10), bd=0, highlightthickness=0)
        self.input_box.pack(fill="x", padx=10, pady=(0, 10))

        # Results table
        table_frame = ctk.CTkFrame(self, fg_color=CARD_BG, border_color=GOLD_DARK, border_width=1)
        table_frame.grid(row=3, column=0, sticky="nsew", padx=20, pady=5)
        table_frame.grid_columnconfigure(0, weight=1)
        table_frame.grid_rowconfigure(1, weight=1)

        search_row = ctk.CTkFrame(table_frame, fg_color="transparent")
        search_row.grid(row=0, column=0, sticky="ew", padx=10, pady=(10, 5))
        ctk.CTkLabel(search_row, text="🔎 Search:", font=("Segoe UI", 10), text_color=GREY_TEXT).pack(side="left")
        self.search_var = tk.StringVar()
        self.search_var.trace_add("write", lambda *a: self._apply_filter())
        ctk.CTkEntry(search_row, textvariable=self.search_var, width=220, fg_color="#161310", border_color=GOLD_DARK, text_color=CREAM).pack(side="left", padx=8)

        self._setup_treeview_style()
        columns = ("no", "tracking", "courier", "status", "location", "update", "eta")
        self.tree = ttk.Treeview(table_frame, columns=columns, show="headings", style="RDX.Treeview")
        headings = {"no": "#", "tracking": "Tracking No", "courier": "Courier", "status": "Status",
                    "location": "Location", "update": "Last Update", "eta": "ETA"}
        widths = {"no": 40, "tracking": 160, "courier": 90, "status": 110, "location": 160, "update": 150, "eta": 90}
        for c in columns:
        ...
        bottom.grid(row=4, column=0, sticky="ew", padx=20, pady=(5, 15))
        bottom.grid_columnconfigure(0, weight=1)
        self.progress = ctk.CTkProgressBar(bottom, fg_color=CARD_BG, progress_color=GOLD_MID)
        self.progress.grid(row=0, column=0, sticky="ew", padx=(0, 10))
        self.progress.set(0)
        self.status_label = ctk.CTkLabel(bottom, text="Ready", font=("Segoe UI", 10), text_color=GREY_TEXT)
    ...
    def _setup_treeview_style(self):
        ...
        try:
            ...
            pass
        style.configure("RDX.Treeview", background=CARD_BG, foreground=CREAM, fieldbackground=CARD_BG,
                         rowheight=26, font=("Segoe UI", 9), borderwidth=0)
        style.configure("RDX.Treeview.Heading", background="#3A2E20", foreground=GOLD_LIGHT,
                         font=("Segoe UI", 9, "bold"), relief="flat")
        style.map("RDX.Treeview", background=[("selected", GOLD_DARK)], foreground=[("selected", WHITE)])

    # ── actions ───────────────────────────────────────────────────────────
    def _back(self):
        self.stop_evt.set()
        if self.on_back:
            self.on_back(self.username, self.user_id)
    ...
    def _parse_input(self):
        ...
        shipments = []
        default_courier = self.courier_var.get()
        for line in raw.splitlines():
            line = line.strip()
            if not line:
                continue
            if ":" in line and line.split(":", 1)[0] in COURIER_LIST:
                courier, tn = line.split(":", 1)
                shipments.append({"tracking_no": tn.strip(), "courier": courier.strip()})
            else:
                shipments.append({"tracking_no": line, "courier": default_courier})
        return shipments

    def _start_tracking(self):
        if self.running:
            return
        shipments = self._parse_input()
        if not shipments:
            messagebox.showwarning("No Input", "Please paste at least one tracking number.")
            return
        self.results = []
        ...
        self.progress.set(0)
        self.status_label.configure(text=f"Tracking 0/{len(shipments)}...")
        threading.Thread(target=self._run_tracking_bg, args=(shipments,), daemon=True).start()

    def _run_tracking_bg(self, shipments):
        engine = TrackingEngine(self._config)

        def on_progress(done, total, result):
            self.after(0, lambda: self._on_progress_ui(done, total, result))

        results = engine.track_bulk(shipments, progress_callback=on_progress, stop_event=self.stop_evt)
        self.after(0, lambda: self._on_tracking_done(results))

    def _on_progress_ui(self, done, total, result):
        self.results.append(result)
        self._add_row(result)
        self.progress.set(done / total if total else 0)
        self.status_label.configure(text=f"Tracking {done}/{total}...")

    def _on_tracking_done(self, results):
        self.running = False
        self.start_btn.configure(state="normal")
        self.stop_btn.configure(state="disabled")
        self.status_label.configure(text=f"Done — {len(results)} tracked")
    ...
    def _add_row(self, r):
        ...
        tag = "delivered" if getattr(r, "delivered", False) else ("error" if r.status in ("Error", "Unknown Courier") else "")
        self.tree.insert("", tk.END, values=(n, r.tracking_no, r.courier, r.status, r.location, r.timestamp, r.eta), tags=(tag,))
        self.tree.tag_configure("delivered", foreground=GREEN_OK)
        self.tree.tag_configure("error", foreground=RED_ERR)
    ...
    def _apply_filter(self):
        ...
        for r in self.results:
            row_text = f"{r.tracking_no} {r.courier} {r.status} {r.location}".lower()
            if q in row_text:
                self._add_row(r)
    ...
    def _show_detail(self, _evt):
        ...
        w.title(f"Detail — {tn}")
        fit_to_screen(w, min_w=480, min_h=380, max_w=800, max_h=650, width_frac=0.4, height_frac=0.5)
        w.configure(fg_color=DARK_BG)
        box = tk.Text(w, bg=CARD_BG, fg=CREAM, insertbackground=GOLD_LIGHT, font=("Consolas", 10), bd=0, highlightthickness=0)
        box.pack(fill="both", expand=True, padx=15, pady=15)
        for k, v in r.to_dict().items():
            box.insert(tk.END, f"{k}: {v}\n")
        if r.events:
            box.insert(tk.END, "\n--- Events ---\n")
            for ev in r.events:
                box.insert(tk.END, f"{ev}\n")
    ...
    def _export_excel(self):
        if not self.results:
            messagebox.showwarning("No Data", "No tracking results to export yet.")
            return
        fp = filedialog.asksaveasfilename(defaultextension=".xlsx", filetypes=[("Excel files", "*.xlsx")],
                                           initialfile=f"courier_tracking_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")
        if not fp:
            return
        try:
            wb = Workbook()
            ...
            for cell in ws[1]:
                ...
                cell.fill = PatternFill("solid", fgColor="8B6914")
            for r in self.results:
                ws.append(list(r.to_dict().values()))
    ...
    def _open_settings(self):
        win = ctk.CTkToplevel(self)
        win.title("Courier API Settings")
        fit_to_screen(win, min_w=480, min_h=420, max_w=700, max_h=750, width_frac=0.35, height_frac=0.6)
        win.configure(fg_color=DARK_BG)
        scroll = ctk.CTkScrollableFrame(win, fg_color=DARK_BG)
        scroll.pack(fill="both", expand=True, padx=15, pady=15)

        fields = list(self._config.keys()) or [
            "fedex_api_key", "fedex_secret_key", "dhl_api_key", "ups_api_key", "ups_client_id",
            "ups_client_secret", "aramex_username", "aramex_password", "aramex_account_no",
            "aramex_account_pin", "aramex_account_entity", "aramex_country_code",
            "shiprocket_email", "shiprocket_password", "onpoint_api_key",
            "shipglobal_api_key", "xindus_api_key", "xindus_client_id",
        ]
        entries = {}
        for key in fields:
            row = ctk.CTkFrame(scroll, fg_color="transparent")
            row.pack(fill="x", pady=4)
            ctk.CTkLabel(row, text=key, width=170, anchor="w", font=("Segoe UI", 10), text_color=GREY_TEXT).pack(side="left")
            show = "*" if "password" in key or "secret" in key or "pin" in key else ""
            var = tk.StringVar(value=str(self._config.get(key, "")))
            ctk.CTkEntry(row, textvariable=var, show=show, fg_color="#161310", border_color=GOLD_DARK, text_color=CREAM).pack(side="left", fill="x", expand=True, padx=(8, 0))
            entries[key] = var
        ...
        ctk.CTkButton(win, text="💾 Save Settings", fg_color=GOLD_MID, text_color=DARK_BG, hover_color=GOLD_LIGHT, command=save).pack(pady=(0, 15))
...
class App(ctk.CTk):
    ...
    def _show_dashboard(self, username, user_id=None, is_admin=False):
        self._switch_frame(
            ...
            on_open_courier_tracker=self._show_courier_tracker,
        )

    def _show_processing(self, username, user_id=None):
        self._switch_frame(ProcessingWindow, username=username, user_id=user_id)

    def _show_courier_tracker(self, username, user_id=None):
        self._switch_frame(CourierTrackerPage, username=username, user_id=user_id, on_back=self._show_dashboard)
